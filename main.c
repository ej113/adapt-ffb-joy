		...
		if (USB_ControlRequest.bmRequestType == (REQDIR_DEVICETOHOST | REQTYPE_CLASS | REQREC_INTERFACE))
		{
			...
			else
			{
				...
				if (sw_id == SW_ID_FFPW)
				{
					...
					Wheel_CreateInputReport(USB_ControlRequest.wValue & 0xFF, &WheelReportData);

					Endpoint_ClearSETUP();

					/* Write the report data to the control endpoint */
					Endpoint_Write_Control_Stream_LE(&WheelReportData, sizeof(USB_WheelReport_Data_t));
					Endpoint_ClearOUT();
				}
				else
				{
...
void HID_Task(void)
{
	...
	if (Endpoint_IsINReady())
	{
		if (sw_id == SW_ID_FFPW)
		{
			...
			/* Create the next HID report to send to the host */
			Wheel_CreateInputReport(INPUT_REPORTID_ALL, &WheelReportData);

			/* Write Wheel Report Data */
			Endpoint_Write_Stream_LE(&WheelReportData, sizeof(USB_WheelReport_Data_t), NULL);
