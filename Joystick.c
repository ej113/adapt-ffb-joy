...
{
	// Read the data from the FFP-Wheel
	...
	*/
	...
	// Convert the raw input data to USB report
	outReportData->reportId = 1;
	// Convert data from Sidewinder Force Feedback wheel
	/*
	Change
	1(A)	to 1
	2(B) to 2
	...
	8(Rear left) to 6
	*/
	//outReportData->Button = ((sw_report[2] << 2 & 0b11000000) | ((sw_report[2] & 0b0000100) >> 1) | ((sw_report[2] & 0b00000100) << 1) | ((sw_report[2] & 0b00000011) << 2) | ((sw_report[3] & 0b10000000) >> 7) | ((sw_report[3] & 0b01000000) >> 4)) ^ 0x00ff;
	outReportData->Button = ((sw_report[2] << 2) | (sw_report[3] >> 6)) ^ 0x00ff;
	outReportData->Steering = ((sw_report[4] & 0x03) << 8) + sw_report[5];
	// Accellerator adjust 16bit 
	if (48 > ((sw_report[4] & 0xfc) >> 2))
	{
		outReportData->Accellerator = 16 * ((sw_report[4] & 0xfc) >> 2);
	}
	else
	{
		outReportData->Accellerator = 17 * ((sw_report[4] & 0xfc) >> 2) - 48;
	}

	outReportData->Brake = sw_report[3] & 0x3f;
